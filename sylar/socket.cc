...
bool Socket::getOption(int level, int option, void* result, size_t* len){
    ...
    if (rt) {
        SYLAR_LOG_DEBUG(g_logger)<<"getOption sock="<<m_sock<<" level="<<level<<" option="<<option<<" errno="<<errno<<
...
bool Socket::bind(const Address::ptr addr){
    ...
    if(::bind(m_sock,addr->getAddr(), addr->getAddrLen())){
        SYLAR_LOG_ERROR(g_logger)<<"bind error errno="<<errno<<" strerr="<<strerror(errno);
        return false;
    }
    getLocalAddress();
    return true;
...
Address::ptr Socket::getRemoteAddress(){
    ...
    if (getpeername(m_sock,result->getAddr(),&addrlen)){\
        SYLAR_LOG_ERROR(g_logger)<<"getpeername error sock="<<m_sock<<" errno="<<errno<<" strerr"<<strerror(errno);
...
Address::ptr Socket::getLocalAddress(){
    ...
    if (getsockname(m_sock,result->getAddr(),&addrlen)){\
        SYLAR_LOG_ERROR(g_logger)<<"getsockname error sock="<<m_sock<<" errno="<<errno<<" strerr"<<strerror(errno);
        return Address::ptr(new UnknownAddress(m_family));
    }
